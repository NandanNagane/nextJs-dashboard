...
export async function fetchRevenue() {
  try {
    ...
    const [data] = await db.query<any>("SELECT * FROM revenue");
...
export async function fetchCardData() {
  try {
    const invoiceCountPromise = db.query<any>(
      "SELECT COUNT(*) AS count FROM invoices",
    );
    const customerCountPromise = db.query<any>(
      "SELECT COUNT(*) AS count FROM customers",
    );
    const invoiceStatusPromise = db.query<any>(`SELECT
